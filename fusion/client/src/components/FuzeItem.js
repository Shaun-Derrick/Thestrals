...
const FuzeItem = (props) => {
  if (
    props.fuzeObject.startDate === " " ||
    props.fuzeObject.startDate === null
  ) {
    return (
    ...
    return (
      <div
        className="fuzeItem ui link card"
        onClick={() => {
          props.openModal()
          props.chooseFuze(props.fuzeObject)
          props.getDateLong(props.fuzeObject.startDate)
        }}
      >
        <div className="image">{props.chooseIcon(props.fuzeObject.tags)}</div>
        <div className="content">
          <h2 className="fuzeTitle header">{props.fuzeObject.Title}</h2>
        </div>
        <div className="fuzeDate image">
          ...
          <div className="date">
            <p className="month">
              {getDateShort(props.fuzeObject.startDate, 1)}
            </p>
            <span className="day">
              {getDateShort(props.fuzeObject.startDate, 2)}
            </span>
          </div>
        </div>
      </div>
